...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('self-desctruct')
        .setDescription('Send a dm that will be deleted after the receiver read it!')
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const info = `Please press the button below to see the message. ⚠️ You have ${timeout} second(s) before the message is deleted as you click reveal.`;
            ...
            collector.on('collect', async (button) => {
                if (button.customId === 'reveal_button' && button.user.id === targetUser.id) {
                    await button.update({ content: message, components: [], ephemeral: true });
